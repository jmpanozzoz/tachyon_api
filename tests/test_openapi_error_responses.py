...
async def test_openapi_includes_422_and_500_error_responses():
    ...
    assert r422["$ref"].endswith("#/components/schemas/ValidationErrorResponse".split("#/",1)[1]) or r422["$ref"].endswith("#/components/schemas/ValidationErrorResponse")

    assert "500" in op["responses"], "500 response must be present"
    r500 = op["responses"]["500"]["content"]["application/json"]["schema"]
    assert r500["$ref"].endswith("#/components/schemas/ResponseValidationError".split("#/",1)[1]) or r500["$ref"].endswith("#/components/schemas/ResponseValidationError")
